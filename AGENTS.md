...
9. **The session is not done until everything is committed AND pushed**, the session is logged in `.context/memory/agents/sessions.md`, and `.context/memory/tasks/current.md` is cleared. Clock out too: remove your row from `.context/memory/agents/roster.md` in the closing memory commit, so the board shows who is in the office now. If the user has to remind you to commit or push, that is a protocol failure — log it in `.context/memory/flaws/log.md`.
