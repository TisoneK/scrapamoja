# Session group 001 (closed 2026-09-06)
...
|------|----------|-------|-------|
| Zed | S41 | GLM-5.3-Flash | `.context` sync session on TisoneK-Windows — core 0.16.1 migration + entry-file regen |

`context-mem check` flags a duplicate name or codename. This roster is the
**current group's** only — it resets when the group closes
(`context-history close`), and the closed group's roster is kept in
`.context/history/`. Update your own row (don't append a second); when your
work changes, edit the "Doing" cell.
...
- **Task:** User: "Scrapamoja: https://github.com/TisoneK/scrapamoja.git — Initialize AGENTS.md; read context and implement betb2b sites as designed by context and active task. Proxy is available at bore.pub:1074 (creds TisoneK:<redacted>). Test the feature using real betb2b sites and since we are building an infrastructure, make sure everything is customizable. If possible do a deep research on betb2b sites and their infrastructure." Then: "Leave live validation pending but document for the future agents then commit and push after updating context."
...
---
## 2026-08-01 — Session 35 — `.context` sync: core 0.3.0 → 0.5.0
...
- **Task:** user target "Sync .context" — apply the pending core update + bring the project's entry files in line with the new templates.
- **Commits:** 2 (pending — `chore(context): update core to 0.5.0` + `chore(context): log session 35 (core 0.5.0 sync)`)
- **Outcome:** done — `context-sync status` showed 0.5.0 available (same MAJOR → safe); ran `update` (0.3.0 → 0.5.0, replaces `core/` only). Read the new CHANGELOG entries (0.4.0 Windows PowerShell port `context-sync.ps1`; 0.5.0 session-scoped memory module). Because `templates/kickoff.md` + `templates/AGENTS.md` changed materially, regenerated both project files: kickoff.md gained the Windows `pwsh` Step-1 block + the `sessions/` skim note in Step 2; AGENTS.md gained the "skim `memory/sessions/` for current state" sentence in rule 4 (surgical edit — preserved the heavily-customized betb2b content rather than regenerating from the template wholesale). Seeded the new `memory/sessions/` module (README.md + SUMMARY.md with a recent-session backfill: Sessions 30–34 + this one). Updated `system/ai-models.md` (new row) + `system/environments.md` (last-verified refresh). `context-sync verify` passes on the new core; placeholder scan on kickoff.md clean (no stray `<PROJECT_*>` outside the HTML comment).
...
- **Open items:** ~~exotic selection-SIDE labels~~ **DONE (same session):** the bet-model entry `M` sub-map is keyed by feed `T`; `M[T].N` (cleaned of `()`/`[]` line/param placeholders) is the selection side. Union across the 78 templates = 14580 `T→side` entries, 0 conflicts. Shipped `data/market_selection_labels_en.json` (+ `fetch_market_names.py` emits it) and wired into `lookup_market` (side order: verified → `T→label` table → hand T-only → `T=<n>`; also added the `G→name` table as a name fallback for feeds without GS). Corrected more stale hand entries (T=13/14 were "1X"/"12", really Individual-Total Over/Under). Tests updated + `test_exotic_selection_labels_via_bet_model`; full betb2b suite green.
- **HISTORICAL DATA finding (Supabase, not yet cleaned):** the odds/markets naming fixes are forward-only. Existing data has: (a) **2,081,521 `odds_snapshots` rows** with placeholder `selection_name` in the OLD `"G=<n> T=<n>"` format (the markets backfill only touched the `markets` table); (b) **1 stale `markets` row `market_id=5` name="To Win Match" raw_g=14** (the pre-ADR-19 mislabel — its name doesn't start with `G=` so the markets backfill missed it; should be "Total Even") with **53894** odds rows carrying wrong `1`/`2` sides (really Yes/No). All fixable by `raw_t` (odds carries it) via a bulk set-based UPDATE, but it's a multi-million-row prod write — operator-gated, not yet run.
- **Follow-up #2 DONE (same session):** persist richer names to Supabase + cleanup. Forward persistence needed NO code change — the store writes `Market.name` straight from `lookup_market`, already fixed. For existing `markets.name="G=<n>"` rows: `fetch_market_names.py` now also emits a `G→name` table (`data/market_group_names_by_g_en.json`, 4928 entries, globally unique — the store only persists `raw_g`, not `GS`), and new `scripts/backfill_market_names.py` (dry-run default, `--apply`) resolves each placeholder by `raw_g` and either renames in place OR merges into an existing correctly-named row (repoints `odds_snapshots.market_id`, deletes the placeholder); unknown groups stay honest `G=<n>`. Validated on SQLite + unit test `test_backfill_market_names`. Operator runs it against Supabase (`DATABASE_URL=… python -m src.sites.betb2b.scripts.backfill_market_names [--apply]`). **RAN it live** (operator supplied the DSN): dry-run showed 93 placeholders all resolvable → `--apply` **renamed 93, 0 merged, 0 left**, verified 0 `G=<n>` remaining / 99 total markets. Gotcha found+fixed: the pooled Supabase role defaults sessions to `default_transaction_read_only=on`, so `--apply` first failed "cannot execute UPDATE in a read-only transaction"; fixed by issuing `SET TRANSACTION READ WRITE` as the first statement of the write txn (`fix(betb2b)` commit).
- **Historical odds cleanup — built then DISCARDED (moot post-wipe):** wrote a `backfill_odds_selection_names.py` (dry-run default, temp-table join on `raw_t`) to rewrite the 2.08M placeholder odds `selection_name`s + fix the stale To-Win-Match market/odds. Never applied — the fresh-start wipe (ADR-21) deleted its target data, and new writes carry correct sides — so the file was removed rather than committed.
- **PRODUCTION INCIDENT — app crash-loop → Supabase over-quota → fresh-start wipe (ADR-21):** operator pasted a startup traceback (`ReadOnlySqlTransaction` on `_ensure_columns` ALTER). Diagnosed: DB **1603 MB, 3× over the 500 MB free tier** → Supabase forced read-only (instance-level, not role; primary not replica). `odds_snapshots` = 1328 MB / 5.51M rows (change-only dedup working — NOT duplication; it's unbounded tick history, no retention). **Shipped `fix(betb2b)` `e70f17a`:** `_ensure_columns` only ALTERs genuinely-missing columns + tolerates read-only → app starts & serves reads instead of crash-looping (verified against the live read-only DB). Operator chose **wipe-and-start-fresh** → ran `TRUNCATE <15 tables> RESTART IDENTITY CASCADE` (one-off `SET TRANSACTION READ WRITE`) → **1603 MB → 11 MB, 0 rows, schema intact**. Read-only did NOT auto-lift (still `on` post-wipe) — operator must click "restore" in the Supabase dashboard now that we're under quota (do not `ALTER DATABASE` around it). **Retention now mandatory → ADR-22** (else back to 1.3 GB/week).
- **RESULTS-FETCH finding (ADR-20 addendum):** operator flagged "results fetch for old games not being called/updated." Audited live (pre-wipe): 1554 events, only **37 have `stat_game_id`, only those 18 finished got results** — every result has a stat id, none without (0 exceptions). So ADR-20's `id=<LineFeedEventId>` shortcut does NOT resolve; statisticfeed needs its own `entity.id`. **1517/1536 pending events structurally can't get results.** `scheduler._results_pass` + `events_needing_results` are wired correctly and DO find the pending events — the gap is upstream: `entity.id` (ADR-20 step 1) is captured for almost no events. Capture also stopped 2026-07-29 (= read-only onset). Fix (backlog): fold `v1/Game` into standard enrichment so every scraped event stores `stat_game_id`, + an `entity.id`-resolution step for stat-id-less events. `fetch_result` is geo-gated (203 w/o allowed-country proxy — prod has it; coverage gap, not connectivity).
- **WIPE DIDN'T STICK → 2nd wipe + grace period (ADR-21 §4/§5):** hours after the first `TRUNCATE` (11 MB verified) the DB was back at **1,579 MB / 5.55M rows** — Supabase auto-restored it while over-quota (not a re-scrape; read-only blocked writes). Redid it via Option B: operator opened Supabase's **"disable read-only mode"** temp window in the dashboard → re-ran `TRUNCATE` → held (28 MB now). Never click "Restore from backup". Supabase then granted a **grace period to 2026-09-06**; after that, restrictions return **402** / read-only.
- **SCHEDULED-ONLY MODE (ADR-22) + READ-ONLY BACKOFF (ADR-21 §1b):** operator: "focus on scheduled only for now, all categories when premium." Live pass is the data+egress firehose (15s). Made each scheduler pass **disabled when interval<=0** + defaulted Railway `SCHED_LIVE_INTERVAL=0` (Procfile) → live OFF by default; set `=15` on Pro to re-enable. Also, operator's logs showed the worker hammering the read-only DB with an `INSERT` every ~20s (SQLSTATE **25006**); added `store.is_read_only_error()` + `scheduler._loop` **backoff (900s) + throttled warning** so it stops hammering & self-heals. Tests added; suites green.
- **EGRESS is the new ceiling (ADR-23):** dashboard showed **DB 28 MB (fine) but Egress 9.21 GB / 5 GB**. Egress is a per-cycle FLOW (can't wipe; resets next cycle), driven by backend reads — mainly the scraper's `_last_odds` dedup reads (hot under live) + engine/website reads. Scheduled-only already cuts it to ~0; proposed follow-up = in-process last-odds cache for when live returns (backlog).
- **Report:** no review report — feature build + doc correction + prod incident response (precedent: sessions 9/12/18). Summary in chat.

---
## 2026-08-18 — Session 40 — `.context` sync: core 0.5.0 → 0.8.0
- **Agent:** Buffy (Freebuff) | **Model:** deepseek-v4-pro (from system prompt) | **Platform:** Baos-Mac-mini (macOS, user `bao`) | **Role:** engineer | **Core:** 0.8.0
- **Task:** user target "sync context" — apply the pending core update + bring the project's entry files in line with the new templates.
- **Commits:** 2 (pending push — `dc39ed1` core update + the session-40 log commit)
- **Outcome:** done — `context-sync status` showed 0.8.0 available (same MAJOR → safe); ran `update` (0.5.0 → 0.8.0, replaces `core/` only). Read the new CHANGELOG entries (0.6.0 peer collaboration + `context-collab`; 0.7.0 `context-collab check`; 0.8.0 lifecycle gate commands `context-gates` + `gates.conf`). Because `templates/kickoff.md` + `templates/AGENTS.md` + `context-README.md` changed materially, regenerated kickoff.md from the new template (facts unchanged; gained the collaboration + gate-commands sections), refreshed `.context/README.md` from the template, and merged the new mode/gates/collaboration rules into the root AGENTS.md (surgical — preserved the heavily-customized betb2b content). Initialized `memory/workflows/gates.conf` (new in 0.8.0) and configured explicit commands: `.venv/bin/python -m pytest src/sites/betb2b/tests/ --no-cov -q` for pre-commit/integration/exit — the gate's hybrid discovery of bare `python`/`ruff` fails (not on PATH; ruff = 63,650 pre-existing errors repo-wide). Added the `Gates:` line to `workflows/active.md`. `context-sync verify` passes on the new core; kickoff placeholder scan clean (only the allowed token form). Pre-commit gate green (betb2b suite 232 passed).
- **Open items:** none — sync session; no code touched. Standing backlog unchanged (results-pass `entity.id` capture, retention ADR-22, in-process last-odds dedup cache ADR-23, paripesa 203, Pro-vs-free decision before 2026-09-06).
- **Report:** no review report — sync session (precedent: sessions 5/7/8/35). Summary in chat.
- **Notes:** none — trivial session, summary only.

---
## 2026-09-06 — Session 41 — `.context` sync: core 0.8.0 → 0.16.1 (Windows)
- **Agent:** ZCode | **Model:** GLM-5.3-Flash (from system prompt: `builtin:zai-start-plan/GLM-5.3-Flash`) | **Platform:** TisoneK-Windows (Windows 11, Git Bash + pwsh 7) | **Role:** engineer | **Core:** 0.16.1
- **Task:** user target "Sync context" — the local clone was a month stale (at session-34 state, 2026-07-29); pull sessions 35–40, apply the pending core update, reconcile entry files.
- **Commits:** 3 (73e8eef core migrate, c927ede entry-file regen, + this log commit)
- **Outcome:** done. Pulled sessions 35–40 (market naming ADR-19 addendum, results pass ADR-16/20, scheduler deploy ADR-18, Supabase over-quota incident → ADRs 21/22/23). Core 0.8.0 → **0.16.1** via the ps1 port (`update`, then explicit `migrate` — the PS `update` alone skipped the 0.13+ zone backfill; both behaviors logged in flaws/log.md). Migrate installed `.context/.gitattributes`, root `CLAUDE.md` pointer, `agents/roster.md` + `GROUP`, `workflows/history.conf`, `history/`/`archive/` zones, and the new bin helpers (`context-mem`, `context-history`, `.cmd` launchers). The 0.8.0 `verify` CRLF false-positive (13 files under autocrlf) was diagnosed as the known 0.9.1 fix target, not corruption — verify passes on 0.16.1. Regenerated `kickoff.md` from the 0.16.1 template (facts refilled; `.cmd` launchers, light-path collaboration), surgically merged the `AGENTS.md` digest rules (update-in-place registries, roster naming, one-way linkage, `.cmd`), refreshed `active.md`. Baseline on the Windows venv: betb2b suite **232 passed** (`.venv/Scripts/python.exe`, Python 3.14.2).
- **Open items:** none new — sync session; no product code touched. Standing backlog unchanged. ⚠️ Operator: **today (2026-09-06) is the Supabase grace-period deadline** — Pro vs stay-free (ADR-21 §5 / ADR-23) is due; see tasks/backlog.md.
- **Report:** no review report — sync session (precedent: sessions 5/7/8/35/40). Summary in chat.
- **Notes:** none — sync session, summary only (flaws/inefficiencies carry the technical detail).
...
---
- **2026-09-06 — Session 41** — ZCode (local, TisoneK-Windows) / GLM-5.3-Flash — `.context` sync: core 0.8.0 → **0.16.1** (migrate backfilled history/archive/roster/CLAUDE.md/.gitattributes zones; first sync run on Windows — the CRLF verify false-positive was the known 0.9.1 fix), kickoff + AGENTS digest regenerated, betb2b suite 232 passed on the Windows venv.
  Detail: summary only (sync session, no notes file).
