...
- **Flaw:** `context-sync migrate`'s "all zones/files present; core verified" confirmation is per-directory, not per-file: this repo reached 0.17.0 with `memory/collaboration/README.md` missing and never tracked, yet every migrate/update run reported complete. The file is required by both editions (Step 3 reading order, item 5), the kickoff template, and the schema. Every session since collaboration existed silently skipped a required read.
- **Symptom:** `ls .context/memory/collaboration/` → only `events/`; `git log --all -- <README path>` → empty (never committed). Meanwhile migrate output claimed "all zones/files present" and verify passed (verify checks core/, not memory/).
- **Root cause:** the backfill's completeness check enumerates zone directories (or checks a partial file list), not every template file it is supposed to install; and `verify` scopes core integrity only, so a missing memory file is invisible to both tools.
- **Suggested fix:** make `backfill_project` assert every template-derived memory file individually (print or warn per missing file), and/or have `migrate`'s final check compare the memory tree against the template manifest file-by-file. (Related but distinct from Session 41's flaw that PS `update` skipped the backfill entirely — that was fixed by 0.16.0's `migrate`; this is the backfill's own completeness check being too coarse.)
- **Status:** open here (fixed on this repo 2026-09-08 by copying the template verbatim → `c4c2e09`); open upstream (check remains per-directory in 0.17.0)
---
## 2026-09-14 — Miles (S446) / qwen3.8-flash (Session 48)
- **Flaw:** a "sync context" session can pass while the vendored core is TWO MAJORS behind, because `context-sync status` probes only *local* sources (sibling clone / `CONTEXT_PKG`) and prints "none reachable … this is fine" — and the editions/kickoff tell the agent to "note it and move on". Nothing in the entry steps requires ever checking the package **remote**; worse, the repo's own memory had the upstream URL frozen at whatever it was at bootstrap (`TisoneK/.context`), so a rename leaves every future session silently checking a dead/redirecting name with no signal.
- **Symptom:** Sessions 46/47 reported "no update available/needed" three times (2026-09-08, 2026-09-14 twice) while upstream was at 0.17/0.18/1.x. Operator had to say "Wrong check remote on /TisoneK/context-ledger" before a real drift check happened; it instantly found a MAJOR (1.1.1 vs 0.17.0).
- **Root cause:** the sync protocol treats an unreachable update source as a benign state ("never fatal"), but never defines how to *establish* the source on a box without a package clone — no step says "clone the package remote recorded in kickoff.md, then status"; and stale-recorded URLs are invisible because the old URL redirects.
- **Suggested fix:** in both editions' core-check step (and the kickoff template's Step 1): when `status` reports no reachable source, the agent must resolve the **recorded upstream URL** (`git ls-remote` at minimum, a sibling clone for a real comparison) once per sync session before declaring sync complete; print the compared versions. Also add a "package renamed" line to `context-sync update`'s output when the source's VERSION major differs twice over.
- **Status:** open upstream (0.17.0 shipped this gap); open here (worked around manually: sibling clone on Lameck-Windows, corrected URLs in kickoff.md/active.md)
