...
def _load_int_map(filename: str) -> Dict[int, str]:
    try:
...
# GS → group name (the precise key). G → group name (coarse; feed carries G
# even on list feeds that omit GS). T → selection side label (Over/Under/Yes/
# No/W1/…). All three are the SPA's own bet-model tables (ADR-19), globally
# unique, regenerated by ``scripts/fetch_market_names.py``.
DEFAULT_MARKET_GROUP_NAMES: Dict[int, str] = _load_int_map("market_group_names_en.json")
DEFAULT_MARKET_GROUP_NAMES_BY_G: Dict[int, str] = _load_int_map("market_group_names_by_g_en.json")
DEFAULT_MARKET_SELECTION_LABELS: Dict[int, str] = _load_int_map("market_selection_labels_en.json")
...
def lookup_market(
    ...
    market_group_names_by_g: Optional[Dict[int, str]] = None,
    market_selection_labels: Optional[Dict[int, str]] = None,
    ...
    authoritative **GS → name** table → **G → name** table (for feeds without
    GS) → T-only market name → G-only group name → ``f"G={g_id}"``.
    Selection-side order: verified maps → the authoritative **T → label** table
    (Over/Under/Yes/No/…) → T-only table → ``f"T={t_id}"``. The verified maps
    win because they carry a *hand-confirmed* selection side. All the ``*_names``
    / ``*_labels`` args default to the shipped bet-model tables (ADR-19).
    """
    ...
    # 2. Resolve the group NAME and the SELECTION side independently from the
    #    authoritative bet-model tables (fall back gracefully; never guess).
    gs_names = market_group_names if market_group_names is not None else DEFAULT_MARKET_GROUP_NAMES
    g_names = market_group_names_by_g if market_group_names_by_g is not None else DEFAULT_MARKET_GROUP_NAMES_BY_G
    sel_labels = market_selection_labels if market_selection_labels is not None else DEFAULT_MARKET_SELECTION_LABELS

    mt = market_types.get(t_id)

    # selection SIDE: authoritative T→label → hand T-only table → placeholder.
    selection = sel_labels.get(t_id)
    if selection is None:
        selection = mt.selection_label if mt is not None else f"T={t_id}"

    # group NAME: GS table → G table → hand T-only name → G-only group → placeholder.
    name: Optional[str] = None
    if gs_id is not None:
        name = gs_names.get(gs_id)
    if name is None:
        name = g_names.get(g_id)
