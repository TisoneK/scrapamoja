...
Template for scraping quotes.toscrape.com using the Site Template Integration Framework.

## Overview

This template provides a complete implementation for scraping quotes.toscrape.com with the following features:
...
```python
from src.sites.quotes.scraper import QuotesScraper

# Initialize scraper
scraper = QuotesScraper(page, selector_engine)
...
# Capture screenshot
screenshot_path = await scraper.capture_quotes_screenshot("context")
...
```python
from src.sites.quotes.config import get_config, update_config
...
```bash
python -m pytest tests/sites/quotes/
```
...
```python
from src.sites.quotes.config import update_config
