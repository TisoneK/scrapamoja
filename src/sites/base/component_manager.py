...
class ComponentManager:
    ...
    async def initialize(self, discovery_paths: List[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("ComponentManager initialized", extra={"component_count": len(self._registered_components)})
            return True
            
        except Exception as e:
            logger.error("Failed to initialize ComponentManager", extra={"error": str(e)})
            return False
    ...
    async def register_component(self, component_class: Type[BaseComponent]) -> bool:
        ...
        try:
            ...
            if component_id in self._registered_components:
                logger.debug("Component already registered", extra={"component_id": component_id})
                return False
            ...
            logger.info("Registered component", extra={"component_id": component_id})
            return True
            
        except Exception as e:
            logger.error("Failed to register component", extra={"component_class": component_class.__name__, "error": str(e)})
            return False
    ...
    async def create_instance(
        ...
        try:
            ...
            logger.debug("Created component instance", extra={"instance_id": instance_id, "component_id": component_id})
            return instance_id
            
        except Exception as e:
            logger.error("Failed to create component instance", extra={"component_id": component_id, "error": str(e)})
            return None
    ...
    async def destroy_instance(self, instance_id: str) -> bool:
        ...
        try:
            instance = self._active_instances.get(instance_id)
            if not instance:
                logger.warning("Instance not found", extra={"instance_id": instance_id})
                return False
            ...
            logger.debug("Destroyed component instance", extra={"instance_id": instance_id})
            return True
            
        except Exception as e:
            logger.error("Failed to destroy instance", extra={"instance_id": instance_id, "error": str(e)})
            return False
    ...
    async def _discover_components(self) -> None:
        ...
        for discovery_path in self._discovery_paths:
            try:
                ...
                logger.error("Error discovering components", extra={"discovery_path": discovery_path, "error": str(e)})
    ...
    async def _inspect_module_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.error("Error inspecting module file", extra={"file_path": str(file_path), "error": str(e)})
    ...
    async def _resolve_dependencies(self, component_id: str) -> bool:
        ...
        for dep in dependencies:
            if dep.component_id not in self._registered_components:
                if dep.optional:
                    logger.debug("Optional dependency not found", extra={"dependency": dep.component_id, "component_id": component_id})
                    continue
                else:
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            logger.error("Error during ComponentManager cleanup", extra={"error": str(e)})
