"""
Adaptive API Client for selector alternative resolution.

This module provides an async HTTP client for calling the adaptive REST API
to retrieve alternative selectors when primary selectors fail.
...
class AlternativeSelector:
    """Represents an alternative selector from the API."""
...
@dataclass
class APIResponse:
    """Response from the adaptive API."""
...
class AdaptiveAPIClient:
    """
    Async HTTP client for the adaptive selector API.
    ...
    Usage:
        ...
        alternatives = await client.get_alternatives("selector_id", "https://example.com")
    """

    _instance: Optional["AdaptiveAPIClient"] = None
    ...
    def __init__(
        ...
        """
        Initialize the AdaptiveAPIClient.
    ...
    async def get_instance(
        ...
        """
        Get or create the singleton instance of AdaptiveAPIClient.
        ...
        Returns:
            The singleton AdaptiveAPIClient instance
        """
    ...
    def _get_logger(self) -> Any:
        ...
        try:
            ...
            return get_logger("adaptive_api_client")
        except ImportError:
            import logging

            return logging.getLogger("adaptive_api_client")
    ...
    async def __aenter__(self) -> "AdaptiveAPIClient":
    ...
    async def get_alternatives(
        ...
        Returns:
            APIResponse containing alternatives or empty list (graceful degradation)
        """
        ...
        if not self.is_service_available():
            ...
            if state == ServiceState.RECOVERING:
                self._logger.info(
                    "adaptive_api_recovering_attempt",
                    extra={
                ...
                self._logger.warning(
                    "adaptive_api_service_unavailable",
                    extra={
                        "selector_id": selector_id,
                        "page_url": page_url,
    ...
    async def health_check(self) -> bool:
        """
        Check if the adaptive API is available.
        ...
        try:
            ...
            if is_healthy:
                ...
                self._logger.info(
                    "adaptive_api_healthy",
                    extra={"base_url": self.base_url},
                )
            else:
                self._availability.record_failure(self.circuit_breaker_threshold)
                self._logger.warning(
                    "adaptive_api_unhealthy",
                    extra={
            ...
            self._availability.record_failure(self.circuit_breaker_threshold)
            self._logger.warning(
                "adaptive_api_health_check_failed",
                extra={
                    "base_url": self.base_url,
                    "error": str(e),
                },
    ...
    def reset_availability(self) -> None:
        ...
        self._logger.info("adaptive_api_availability_reset")
...
class AdaptiveAPIClientConfig:
    """
    Configuration dataclass for AdaptiveAPIClient.

    This provides a clean way to configure the client with all options.
    """
