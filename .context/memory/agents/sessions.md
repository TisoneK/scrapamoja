...
- **Follow-up #2 DONE (same session):** persist richer names to Supabase + cleanup. Forward persistence needed NO code change — the store writes `Market.name` straight from `lookup_market`, already fixed. For existing `markets.name="G=<n>"` rows: `fetch_market_names.py` now also emits a `G→name` table (`data/market_group_names_by_g_en.json`, 4928 entries, globally unique — the store only persists `raw_g`, not `GS`), and new `scripts/backfill_market_names.py` (dry-run default, `--apply`) resolves each placeholder by `raw_g` and either renames in place OR merges into an existing correctly-named row (repoints `odds_snapshots.market_id`, deletes the placeholder); unknown groups stay honest `G=<n>`. Validated on SQLite + unit test `test_backfill_market_names`. Operator runs it against Supabase (`DATABASE_URL=… python -m src.sites.betb2b.scripts.backfill_market_names [--apply]`) — I have no `DATABASE_URL` here and prod UPDATEs are operator-gated.
