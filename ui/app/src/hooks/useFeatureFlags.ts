...
import { FeatureFlag, FeatureFlagListResponse, FeatureFlagStatsResponse } from '@/types/featureFlag'
...
export function useFeatureFlags(filterOptions: FilterOptions = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const data = await apiClient.getFeatureFlags(filterOptions.sport, filterOptions.site)
        return data
      } catch (error) {
    ...
    select: (data: FeatureFlagListResponse) => {
      ...
      if (filterOptions.sortBy) {
        filteredFlags.sort((a, b) => {
          ...
          // Handle date sorting
          if (filterOptions.sortBy === 'updated_at' || filterOptions.sortBy === 'created_at') {
    ...
    onError: (error) => {
      ...
      // toast.error('Failed to load feature flags. Please refresh the page.')
    },
  })
}
...
export function useToggleFeatureFlag() {
  ...
  return useMutation({
    mutationFn: async ({ sport, site, enabled }: { sport: string; site?: string; enabled: boolean }) => {
      try {
...
export function useCreateFeatureFlag() {
  ...
  return useMutation({
    mutationFn: async ({ sport, site, enabled, description }: { 
      sport: string; 
      site?: string; 
      enabled: boolean; 
      description?: string 
