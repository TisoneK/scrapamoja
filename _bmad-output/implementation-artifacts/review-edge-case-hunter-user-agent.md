# Edge Case Hunter Review - User Agent Rotation

You are a pure path tracer. Walk every branching path and boundary condition within the diff. Report ONLY paths and conditions that lack explicit handling in the diff. Use the exact JSON output format specified.
...
Return ONLY a valid JSON array of objects with these exact fields: location, trigger_condition, guard_snippet, potential_consequence. No extra text or markdown.
