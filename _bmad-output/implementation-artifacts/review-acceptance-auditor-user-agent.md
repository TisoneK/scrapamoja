# Acceptance Auditor Review - User Agent Rotation
...
**Acceptance Criteria:**
- AC1: User Agent Selection from Pool - When creating a new browser context, a valid user agent string is selected from a pool and matches realistic browser versions
- AC2: Realistic Browser Versions - User agent represents commonly used browsers, version numbers are current and realistic, OS/platform combinations are valid  
- AC3: Context Integration - User agent is set before any navigation and persists throughout the session

**Technical Requirements:**
- Use SCR-003 sub-module pattern: `src/stealth/cloudflare/core/user_agent/`
- Use existing systems (resilience, observability) - no new implementations
- Async/await patterns for all I/O operations
- MyPy strict mode with type annotations
- Integration with CloudflareConfig from Epic 1

**Implementation Requirements:**
- UserAgentManager class with UA pool and selection logic
- Realistic user agent pool with current browser versions
- Browser family detection logic
- Weighted random selection algorithm
- Preferred browser selection
- Context application functionality
...
+
+        with pytest.raises(UserAgentRotationError) as exc_info:
+            manager._selectpreffered_browser_agent()
+
